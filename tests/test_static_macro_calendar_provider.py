...
import modules.macro_calendar_provider as macro_calendar_provider_module
...
def freeze_macro_calendar_time(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        macro_calendar_provider_module,
        "datetime",
        FrozenDateTime,
    )
...
def test_fetch_upcoming_events_returns_deterministic_collection() -> None:
    ...
    first = provider.fetch_upcoming_events(
        regions=[MacroRegion.US],
        days_ahead=365,
    )
    second = provider.fetch_upcoming_events(
        regions=[MacroRegion.US],
        days_ahead=365,
    )
...
def test_fetch_upcoming_events_use_expected_types_and_regions() -> None:
    ...
    assert (
        event_map["FOMC Rate Decision"].event_type
        is MacroEventType.INTEREST_RATE_DECISION
    )
    assert (
        event_map["FOMC Rate Decision"].country
        is MacroRegion.US
    )
    assert (
        event_map["CPI"].event_type
        is MacroEventType.CPI
    )
    assert (
        event_map["CPI"].country
        is MacroRegion.US
    )
    assert (
        event_map["Core CPI"].event_type
        is MacroEventType.CORE_CPI
    )
    assert (
        event_map["Core CPI"].country
        is MacroRegion.US
    )
    assert (
        event_map["PPI"].event_type
        is MacroEventType.PPI
    )
    assert (
        event_map["PPI"].country
        is MacroRegion.US
    )
    assert (
        event_map["Core PPI"].event_type
        is MacroEventType.CORE_PPI
    )
    assert (
        event_map["Core PPI"].country
        is MacroRegion.US
    )
    assert (
        event_map["Non-Farm Payrolls"].event_type
        is MacroEventType.NONFARM_PAYROLLS
    )
    assert (
        event_map["Non-Farm Payrolls"].country
        is MacroRegion.US
    )
    assert (
        event_map["Unemployment Rate"].event_type
        is MacroEventType.UNEMPLOYMENT_RATE
    )
    assert (
        event_map["Unemployment Rate"].country
        is MacroRegion.US
    )
    assert (
        event_map["Core PCE"].event_type
        is MacroEventType.CORE_PCE
    )
    assert (
        event_map["Core PCE"].country
        is MacroRegion.US
    )
